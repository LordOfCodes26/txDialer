...
import com.android.dialer.dialogs.DynamicBottomSheetChooserDialog
...
//        if (config.flashForAlerts) MyCameraImpl.newInstance(this).toggleSOS()
    ...
    private fun addLockScreenFlags() {
        ...
        try {
            ...
            screenOnWakeLock = powerManager.newWakeLock(PowerManager.FULL_WAKE_LOCK, "com.android.dialer:full_wake_lock")
    ...
    private fun enableProximitySensor() {
        if (!config.disableProximitySensor && (proximityWakeLock == null || proximityWakeLock?.isHeld == false)) {
            val powerManager = getSystemService(POWER_SERVICE) as PowerManager
            proximityWakeLock = powerManager.newWakeLock(PowerManager.PROXIMITY_SCREEN_OFF_WAKE_LOCK, "com.android.dialer:wake_lock")
